...
export default class PianoRollStore {
  ...
  get contentWidth(): number {
    ...
    const trackEndTick = this.rootStore.song2.endOfSong
  ...
  get selectedTrack(): Track | undefined {
    return this.rootStore.song2.tracks[this.selectedTrackId]
  }
  ...
  get ghostNotes(): PianoNoteItem[] {
    const song = this.rootStore.song2
    const {
      transform,
      notGhostTracks,
      scrollLeft,
  ...
  get currentTempo(): number | undefined {
    return this.rootStore.song2.conductorTrack?.getTempo(
